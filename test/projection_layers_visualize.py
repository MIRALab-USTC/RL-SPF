...
# visualize the output of every layer of projection
    ...
    for env_name in env_names:
        ...
        with tf.device("/gpu:{}".format(0)):
            with tf.GradientTape(persistent=True) as tape:
                ...
                fourier_pred_re_proj = tf.squeeze(extractor.projection(fourier_pred_re, if_record=True, index=0, training=False))
                fourier_pred_im_proj = tf.squeeze(extractor.projection(fourier_pred_im, if_record=True, index=1, training=False))
                fourier_true_re_proj = tf.squeeze(extractor.projection(fourier_true_re, if_record=True, index=2, training=False))
                fourier_true_im_proj = tf.squeeze(extractor.projection(fourier_true_im, if_record=True, index=3, training=False))


                # fourier_pred_re_proj = []
                # fourier_pred_im_proj = []
                # fourier_true_re_proj = []
                # fourier_true_im_proj = []
                
                # import ipdb
                # ipdb.set_trace()
                
                # 只能在projection内部设置断点，然后断点时将数值保存下来
                # projection_input = tf.keras.layers.Input(shape=(128,17), dtype=tf.float32, name="projection_input")
                # sub_model_proj1 = tf.keras.models.Model(inputs = extractor.projection.input, outputs = extractor.projection.get_layer('projection1').output)
                # fourier_pred_re = sub_model_proj1.predict(fourier_pred_re)
                # fourier_pred_re_proj.append(fourier_pred_re)
                # fourier_pred_im = sub_model_proj1.predict(fourier_pred_im)
                # fourier_pred_im_proj.append(fourier_pred_im)
                # fourier_true_re = sub_model_proj1.predict(fourier_true_re)
                # fourier_true_re_proj.append(fourier_true_re)
                # fourier_true_im = sub_model_proj1.predict(fourier_true_im)
                # fourier_true_im_proj.append(fourier_true_im)

                # sub_model_bn = tf.keras.models.Model(inputs = extractor.projection.input, outputs = extractor.projection.get_layer('batch_normalization_17').output)
                # fourier_pred_re = sub_model_bn.predict(fourier_pred_re)
                # fourier_pred_re_proj.append(fourier_pred_re)
                # fourier_pred_im = sub_model_bn.predict(fourier_pred_im)
                # fourier_pred_im_proj.append(fourier_pred_im)
                # fourier_true_re = sub_model_bn.predict(fourier_true_re)
                # fourier_true_re_proj.append(fourier_true_re)
                # fourier_true_im = sub_model_bn.predict(fourier_true_im)
                # fourier_true_im_proj.append(fourier_true_im)

                # sub_model_proj2 = tf.keras.models.Model(inputs = extractor.projection.input, outputs = extractor.projection.get_layer('projection2').output)
                # fourier_pred_re = sub_model_proj2.predict(fourier_pred_re)
                # fourier_pred_re_proj.append(fourier_pred_re)
                # fourier_pred_im = sub_model_proj2.predict(fourier_pred_im)
                # fourier_pred_im_proj.append(fourier_pred_im)
                # fourier_true_re = sub_model_proj2.predict(fourier_true_re)
                # fourier_true_re_proj.append(fourier_true_re)
                # fourier_true_im = sub_model_proj2.predict(fourier_true_im)
                # fourier_true_im_proj.append(fourier_true_im)
        ...
        layers = ['projection1', 'batch_normalization_17', 'projection2']
            ...
            for type0 in types:
                ...
                if complex_type == 'real':
                    if type0 == 'true and predicted':
                        ...
                        # ax.plot(x, fourier_true_re_proj[i,0,:], linewidth=2.5)
                        # ax.plot(x, fourier_pred_re_proj[i,0,:], linewidth=1)
                        # ax.legend(['true', 'pred'])

                        ax.plot(ratio * np.arange(args.dim_discretize), tf.squeeze(fourier_true_re)[:, 0])
                        ax.plot(ratio * np.arange(args.dim_discretize), tf.squeeze(fourier_pred_re)[:, 0])
                        ax.legend(['true', 'pred'])

                        # ax.legend(['true_proj', 'pred_proj', 'true', 'pred'])
                        # plt.ylim((0, 0.1))
                    elif type0 == 'csError':
                        print('cosine similarity of the projection of {}\'s fourier {} is {}\n'.format(env_name.split("-")[0], complex_type, \
                                                                                                    loss_fun(fourier_pred_re_proj, fourier_true_re_proj)))
                    ...
                    if type0 == 'true and predicted':
                        ...
                        # ax.plot(x, fourier_true_im_proj[i,0,:], linewidth=2.5)
                        # ax.plot(x, fourier_pred_im_proj[i,0,:], linewidth=1)
                        # ax.legend(['true', 'pred'])

                        ax.plot(ratio * np.arange(args.dim_discretize), tf.squeeze(fourier_true_im)[:, 0])
                        ax.plot(ratio * np.arange(args.dim_discretize), tf.squeeze(fourier_pred_im)[:, 0])
                        ax.legend(['true', 'pred'])
                    elif type0 == 'csError':
                        print('cosine similarity of the projection of {}\'s fourier {} is {}\n'.format(env_name.split("-")[0], complex_type, \
                                                                                                    loss_fun(fourier_pred_im_proj, fourier_true_im_proj)))
                
                ax.set_xlabel('Omega')
                ax.set_ylabel('fourier {}'.format(complex_type))
                ax.set_title('the projection of {}\'s fourier {}\n'.format(env_name.split("-")[0], complex_type))

                plt.savefig(os.path.join(img_save_dir, dir_of_env[env_name], '{}\'s_{}_fourier_{}11111'.format(dir_of_env[env_name], type0, complex_type)), dpi=300)
                plt.close ('all')
