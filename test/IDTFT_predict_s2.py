...
from teflon.util.misc import get_target_dim, make_ofe_name, parse_cmdline_kwargs
...
def main():
    ...
    for env_name in env_names:
        ...
        for n in range(1, 8):
            ...
            types = ['true and predicted', 'error']
            complex_types = ['s2']
            for complex_type in complex_types:
                
                for type0 in types:
                    
                    if not os.path.exists(os.path.join(img_save_dir, img_save_sub_dir, '{}_next_state_{}'.format(type0, n))):
                        os.mkdir(os.path.join(img_save_dir, img_save_sub_dir, '{}_next_state_{}'.format(type0, n)))
                    ...
                    if type0 == 'true and predicted':
                        for i in range(dim_state):
                            fig, ax = plt.subplots()
                            ax.plot(np.arange(1, horizon), s_future_data[:, i])
                            ax.plot(np.arange(1, horizon), inv_s2[:, i])
                            ax.legend(['true', 'pred'])
                            ax.set_xlabel('horizon')
                            ax.set_ylabel('next state {}'.format(i))
                            ax.set_title('{}\'s the {}th next step state {}'.format(env_name.split("-")[0], n, i))
                            plt.savefig(os.path.join(img_save_dir, img_save_sub_dir, '{}_next_state_{}'.format(type0, n), '{}_next_state_{}'.format(dir_of_env[env_name], i)), dpi=300)
                            plt.close ('all')   
                        ...
                        ax.legend(['error'])
                        ax.set_xlabel('horizon')
                        ax.set_ylabel('next state {}'.format(i, complex_type))
