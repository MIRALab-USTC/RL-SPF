...
def main():
    ...
    for env_name in env_names:
        ...
        with tf.device("/gpu:{}".format(0)):
            ...
            end = args.dim_discretize  # int(args.dim_discretize*0.5 + 1), args.dim_discretize  # predict fourier function in [0,\pi]
            ...
            fourier_pred_mod = tf.concat([fourier_pred_mod, fourier_pred_mod[int(args.dim_discretize*0.5-1): 0: -1]], axis=0)
            fourier_pred_angle = tf.concat([fourier_pred_angle, fourier_pred_angle[int(args.dim_discretize*0.5-1): 0: -1, :]], axis=0)
                ...
                for i in range(dim_state):
                    ...
                    if complex_type == 'module':
                        if type0 == 'true and predicted':
                            ax.plot(ratio * np.arange(end), fourier_true_mod[:, i], linewidth=2.5)
                            ax.plot(ratio * np.arange(end), fourier_pred_mod[:, i], linewidth=2.5)
                            ax.legend(['true', 'pred'], fontsize=15, loc='upper right')
                            # plt.ylim((0, 0.1))
                        elif type0 == 'error':
                            ax.plot(ratio * np.arange(end), fourier_error_mod[:, i], linewidth=2.5)
                            ax.legend(['error'], fontsize=15, loc='upper right')

                    elif complex_type == 'angle':
                        if type0 == 'true and predicted':
                            ax.plot(ratio * np.arange(end), fourier_true_angle[:, i], linewidth=2.5)
                            ax.plot(ratio * np.arange(end), fourier_pred_angle[:, i], linewidth=2.5)
                            ax.legend(['true', 'pred'], fontsize=15, loc='upper right')
                        elif type0 == 'error':
                            ax.plot(ratio * np.arange(end), fourier_error_angle[:, i], linewidth=2.5)
                            ax.legend(['error'], fontsize=15, loc='upper right')
                    
                    ax.set_xlabel('Frequency '+r'$\omega$' , fontsize=20)
                    ax.set_xticks([0, 1.57079633, 3.14159265, 4.71238898, 6.28318531])
                    ax.set_xticklabels(['0', entitydefs['pi']+'/2', entitydefs['pi'], '2'+ entitydefs['pi']+'/3', '2'+ entitydefs['pi']])
                    ax.set_yticks([])
                    ax.set_ylabel('state{}\'s fourier {}'.format(i, complex_type), fontsize=20)
                    plt.xticks(fontsize=15)
                    plt.yticks(fontsize=15)
                    plt.title('{}\'s state{} FT {}'.format(env_name.split("-")[0], i, complex_type), fontsize=25) #  of the last {} states in an episode
                    plt.tight_layout()
