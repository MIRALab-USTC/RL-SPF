...
from src.util.misc import get_target_dim, make_ofe_name, parse_cmdline_kwargs
...
def main():
    ...
    for env_name in env_names:
        ...
        if extra_args['data_source'] == 'replay_buffer':
            img_save_sub_dir = policy_name + '-' + algo + '-' + dir_of_env[env_name] + extra_args['suffix'] + '_rb'
            ...
            try:
                ...
                continue
            exp_data_len = len(exp_data['states'])
            states = exp_data['states']
            actions = exp_data['actions']
            # s_future_data = exp_data['states'][exp_data_len - horizon + 1: exp_data_len]
            # input_s = exp_data['states'][exp_data_len - horizon].reshape(1, -1)
            # input_a = exp_data['actions'][exp_data_len - horizon].reshape(1, -1)
             
        elif extra_args['data_source'] == 'evaluation':
            img_save_sub_dir = policy_name + '-' + algo + '-' + dir_of_env[env_name] + extra_args['suffix'] + '_eval'
            if not os.path.exists(os.path.join(img_save_dir, img_save_sub_dir)):
                os.mkdir(os.path.join(img_save_dir, img_save_sub_dir))
            try:
            ...
            states = tf.cast(np.array(exp_data.iloc[:, 1: dim_state + 1]), dtype=tf.float32)
            actions = tf.cast(np.array(exp_data.iloc[:, dim_state + 1: dim_state + dim_action + 1]), dtype=tf.float32)
        ...
        s_future_data = states[exp_data_len - horizon + 1: exp_data_len, :]
        s_future_data = s_future_data - tf.reduce_mean(s_future_data, axis=0)
        s_future_data = s_future_data / tf.tile(tf.expand_dims(norm(s_future_data, axis=0), axis = 0), multiples=[horizon-1, 1])
        # color_list = ['#96092B','#B20C34', '#D41042', '#EC114B', '#F13E78', '#F88DB9', '#FBBFD9']
        color_list = ['#96092B', '#D52728', '#F13E78', '#F88DB9', '#FBBFD9']
        dash_list = [20, 15, 10, 5, 2]  # [2, 5, 10, 15, 20]
        if args.env == 'Ant-v2':
            dim_state = 27
        elif args.env == 'Humanoid-v2':
            dim_state = 292
        for i in range(dim_state):
            fig, ax = plt.subplots(figsize=(9,6.5))  # figsize=(12,6)
            ax.plot(np.arange(1, horizon), s_future_data[:, i], linewidth=2)
            legend = ['true']
            for n in range(1, 6):
                legend.append('{}-step pred'.format(str(n)))

                input_s = states[exp_data_len - horizon + 1 - n: exp_data_len - n, :]
                input_a = actions[exp_data_len - horizon + 1 - n: exp_data_len - n, :]

                with tf.device("/gpu:{}".format(0)):
                    fourier_pred_re, fourier_pred_im = extractor([input_s, input_a])

                    end = int(args.dim_discretize*0.5 + 1) # int(args.dim_discretize*0.5 + 1), args.dim_discretize
                    fourier_pred_re = fourier_pred_re[:, 0:end, :]
                    fourier_pred_im = fourier_pred_im[:, 0:end, :]
                    ratio = 2 * np.pi / args.dim_discretize
                    ratio2 = 1.0/(2*np.pi*args.dim_discretize)
                    con_re = tf.constant([1] + [2*math.cos(k * ratio * n) for k in range(1, end-1)] + [math.pow(-1, i)])
                    con_re = tf.expand_dims(ratio2 * con_re, axis = 0)
                    con_im = tf.constant([0] + [-2*math.sin(k * ratio * n) for k in range(1, end-1)] + [0])
                    con_im = tf.expand_dims(ratio2 * con_im, axis = 0)
                    inv_s2 = tf.squeeze(tf.matmul(con_re, fourier_pred_re) + tf.matmul(con_im, fourier_pred_im), axis=1)
                    # normalization
                    # inv_s2 = inv_s2 / tf.tile(tf.expand_dims(norm(inv_s2, axis=0), axis = 0), multiples=[horizon-1, 1])
                    # s_future_data = s_future_data / tf.tile(tf.expand_dims(norm(s_future_data, axis=0), axis = 0), multiples=[horizon-1, 1])
                    inv_s2 = inv_s2 - tf.reduce_mean(inv_s2, axis=0)  
                    inv_s2 = inv_s2 / tf.tile(tf.expand_dims(norm(inv_s2, axis=0), axis = 0), multiples=[horizon-1, 1])
                    cosine = np.sum(inv_s2*s_future_data, axis=1)
                    # cosine = np.sum(inv_s2*s_future_data, axis=1)/(norm(inv_s2, axis=1)*norm(s_future_data, axis=1))

                types = ['true and predicted']
                complex_types = ['s2']
                for complex_type in complex_types:
                    
                    for type0 in types:
                        
                        if not os.path.exists(os.path.join(img_save_dir, img_save_sub_dir, '{}_next_state'.format(type0))):
                            os.mkdir(os.path.join(img_save_dir, img_save_sub_dir, '{}_next_state'.format(type0)))

                        logger.info('Drawing {}\'s next state {}\n'.format(env_name.split("-")[0], n))
                        ax.plot(np.arange(1, horizon), inv_s2[:, i] - 0.2*n, color=color_list[n-1], linestyle='--', dashes=(dash_list[n-1],1), linewidth=2.5)  #  , linewidth=3 - 2/7*n
                        # data = {"horizon": np.arange(1, horizon), "next_state": inv_s2[:, i]}
                        # df = pd.DataFrame(data)
                        # with sns.light_palette((20, 60, 50), input="husl"):
                        #     sns.lineplot(data=df. x="horizon", y="next_state")

                        # elif type0 == 'error':
                        #     fig, ax = plt.subplots()
                        #     ax.plot(np.arange(1, horizon), cosine)
                        #     ax.legend(['error'])
                        #     ax.set_xlabel('horizon')
                        #     ax.set_ylabel('next state {}'.format(i))
                        #     ax.set_title('prediction error of {}\'s {}th next state'.format(env_name.split("-")[0], n))
                        #     plt.savefig(os.path.join(img_save_dir, img_save_sub_dir, '{}_next_state_{}'.format(type0, n), '{}_next_state_{}'.format(dir_of_env[env_name], type0)), dpi=300)

                        #     plt.close ('all')   


            plt.title('{}\'s state {}'.format(env_name.split("-")[0],i), fontsize=35) # paper:35
            plt.xlabel('Horizon', fontsize=30)  # paper:35
            plt.ylabel('Future states', fontsize=30)  # paper: 35
            ax.set_xticks([0,50,100,150,200]) 
            ax.set_yticks([])
            plt.xticks(fontsize=20)  # paper: 20
            plt.yticks(fontsize=20)  # paper: 20
            
            # if i==14:
                # ax.legend(legend, bbox_to_anchor=(0.5, -0.4), loc=8, fontsize=20, ncol=6)
            plt.grid()
            plt.tight_layout()
            plt.savefig(os.path.join(img_save_dir, img_save_sub_dir, '{}_next_state'.format(type0), '{}_next_state_{}.png'.format(dir_of_env[env_name], i)), dpi=300)
            plt.close ('all')   
