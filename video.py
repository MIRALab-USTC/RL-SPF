...
from teflon.util.misc import get_target_dim, make_ofe_name, get_default_steps, parse_cmdline_kwargs
...
def save_frames_as_gif(frames, dir_root='./results/video', filename='gym_animation.gif'):
    ...
    anim.save(os.path.join(dir_root, policy+'_'+env+'_animation.gif'), writer='imagemagick', fps=60)

def evaluate_policy(env, policy, eval_episodes=1, record_state=False):
    avg_reward = 0.
    episode_length = []
    state_list = []
